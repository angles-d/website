...
const content = (
    <>
        ...
        <p>
            ...
            This method finds the pixel patches that best match the area (the examplar) in the target region, iteratively filling along the frontier (pixels at the edge of the target and image) to remove the original portion of the image and inpaint the missing section.
        </p>
        ...
        <p>
            To perform the inpainting, each pixel along the frontier is given a priority score using the pixel's confidence which is determined by it's distance from the original image and the strength of it's gradient. Once the highest priority pixel is determined, it's surrounding patch is filled using the examplar patch from the original image which has the highest similarity score.
        </p>
        <div className='my-3'></div>
        <p>
            Below is the full inpainting pipeline with the before and after image on the right.
        </p>
        ...
        <p>It was rather interesting to see how much could be accomplished with just gradients and similarity scores. In the age of GenAI and massive models, it was a good reminder how much the classical computer vision algorithms can still accomplish</p>
        ...
        <p>
            Energy is defined as the difference between the current pixel and it's right and left neighbors which is calculated using the X and Y gradients. For each row of pixels the pixel with the lowest energy is determined. This line of pixels forms a seam. Then the seam with the lowest energy is removed from the image.
        </p>
        ...
        The same technique of seam identification can be used to increase an image size instead. Like cropping stretching an image leads to undesired artifacting; however, by inserting low energy seams we can increase the image size with much less noticeable distortion.
        <div className='flex my-4 mx-auto justify-between'>
            <div>
                <img
                    className="img-project  h-auto"
        ...
        <p>
            In some scenarios only considering the difference between the current pixel and it's neighbors is not sufficent.</p>
        <div className='flex my-4 mx-auto justify-between'>
            <div>
                <img
                    className="img-project  h-auto"
        ...
        <p> To solve this Rubenstein et al. in  <a href="https://faculty.runi.ac.il/arik/scweb/vidret/index.html" target="_blank" className="underline">
            ...
            Unlike the original method (backward energy), to calculate forward energy we look forward and find the difference between neighboring pixels after removing the current pixel.
        </p>
